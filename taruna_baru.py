...
# ============================================================
# INISIALISASI SUPABASE & LOAD MODEL ML
# ============================================================
...
def init_supabase() -> Client:
    try:
        ...
        st.error(f"Gagal koneksi ke Supabase: {e}\nPastikan .streamlit/secrets.toml sudah dikonfigurasi.")
        st.stop()
...
def load_ml_assets():
    try:
        ...
        st.error(f"Gagal meload aset ML. Pastikan 'model_lstm.keras' dan 'scaler.pkl' ada di folder ini. Error: {e}")
        st.stop()

@st.cache_data(ttl=60) # Caching query database selama 1 menit
def fetch_data_from_supabase(_supabase: Client):
    ...
    # Mapping kolom dari database ke format dashboard (Supabase -> Pandas)
    ...
    df["Keterangan"] = df["description"] if "description" in df.columns else ""
    df["Kategori"] = df["category"] if "category" in df.columns else "Umum"
    
    # Feature engineering waktu untuk Visualisasi
    ...
    # Kalkulasi Saldo kumulatif per baris
...
# ============================================================
# PREDIKSI LSTM (VERSI LOAD FILE LOKAL)
# ============================================================

@st.cache_data(show_spinner="🧠 Model LSTM sedang memprediksi data real-time...")
def prediksi_lstm(df_asli: pd.DataFrame, last_bln: int, tahun: int, last_sal: float) -> dict:
    """Prediksi menggunakan model Pre-Trained (keras & pkl) berbasis data Saldo Univariat"""
    
    model, scaler = load_ml_assets()
    TIME_STEPS = 30
    
    # 1. Konversi ke data harian untuk prediksi
    df_harian = df_asli.groupby('Tanggal').agg({'Pemasukan':'sum', 'Pengeluaran':'sum'}).reset_index()
    ...
    # FALLBACK: Jika data harian kurang dari TIME_STEPS
    if n_data < TIME_STEPS:
    ...
    # 2. Scaling Data Saldo (Sesuai instruksi: univariate prediksi saldo)
    saldo_vals = df_harian[["saldo"]].values
    scaled_saldo = scaler.transform(saldo_vals)
    last_days = scaled_saldo[-TIME_STEPS:]
    current_batch = last_days.reshape((1, TIME_STEPS, 1))
    
    # 3. Looping Prediksi 90 Hari ke Depan
    future_preds = []
    for _ in range(90):
        p = model.predict(current_batch, verbose=0)
        future_preds.append(p[0,0])
        current_batch = np.append(current_batch[:, 1:, :], [[[p[0,0]]]], axis=1)
        
    # 4. Kembalikan Skala Nilai (Inverse Transform)
    future_predictions = np.array(future_preds).reshape(-1,1)
    future_rp = scaler.inverse_transform(future_predictions).flatten()
    
    # 5. Agregasikan Prediksi Saldo ke UI Komponen (Pemasukan & Pengeluaran)
    saldo_awal = float(last_sal)
    for i in range(3):
        # Ambil nilai saldo di akhir bulan tersebut
        saldo_akhir_bulan = float(future_rp[(i+1)*30 - 1])
        surplus_estimasi = saldo_akhir_bulan - saldo_awal
        saldo_awal = saldo_akhir_bulan
        
        bn_nama, bn_num = next_month_name(last_bln, i+1)
        
        # Karena LSTM hanya memprediksi Saldo (Univariate), kita proyeksikan pemasukan 
        # dan pengeluaran menggunakan surplus estimasi + historical rata-rata.
        est_pemasukan = max(0, avg_pem_hist + (surplus_estimasi / 2))
        est_pengeluaran = max(0, avg_pen_hist - (surplus_estimasi / 2))
        
        fc3.append({
            'bulan': bn_nama, 'bulan_num': bn_num,
            'pemasukan': round(est_pemasukan), 'pengeluaran': round(est_pengeluaran),
            'surplus': round(surplus_estimasi), 'saldo_akhir': round(saldo_akhir_bulan),
            'ci_pem_80': round(est_pemasukan * 0.12), # Rentang simpangan
            'ci_pen_80': round(est_pengeluaran * 0.12)
        })
    ...
    return {
        ...
        'model': "LSTM Univariate Pre-Trained",
        'n_bulan': n_data // 30,
...
# ============================================================
# SIDEBAR (DISESUAIKAN UNTUK SUPABASE)
# ============================================================
...
def render_sidebar(df):
    with st.sidebar:
        ...
        <div class="sidebar-logo">
            ...
            <span class="logo-sub">Sistem Cerdas ML v4.1</span>
        </div>
        """, unsafe_allow_html=True)
        ...
        if not df.empty:
            ...
            <div class="sidebar-nav">
                ...
                <strong>Prediksi</strong> — AI Forecast<br>
        ...
        <div class="sidebar-footer">
            Karang Taruna &nbsp;·&nbsp; Sistem Prediksi AI Real-time
        </div>""", unsafe_allow_html=True)
        
    return tahun_sel, bulan_sel

# ============================================================
# WELCOME SCREEN (UPDATE TEKS SUPABASE)
# ============================================================
...
def render_welcome():
    ...
    <div class="welcome-box">
        ...
        <div class="welcome-title">Selamat Datang di SIKARTA AI Dashboard</div>
        <div class="welcome-desc">
            Dashboard cerdas ini mengambil data transaksi secara <strong>Real-time dari Supabase</strong> 
            dan memprediksi masa depan keuangan organisasi menggunakan Machine Learning.
        </div>
        <div class="welcome-steps">
            <div class="step-item">
                <div class="step-num">1</div>
                <div class="step-title">Koneksi Otomatis</div>
                <div class="step-desc">Tidak perlu upload Excel. Data langsung tersinkronisasi dari database operasional Anda.</div>
            </div>
            <div class="step-item">
                <div class="step-num">2</div>
                <div class="step-title">Prediksi LSTM</div>
                <div class="step-desc">AI menganalisis pola saldo masa lalu untuk memprediksi kesehatan kas 3 bulan ke depan.</div>
            </div>
            <div class="step-item">
                <div class="step-num">3</div>
                <div class="step-title">Transparansi Data</div>
                <div class="step-desc">Warga dan pengurus dapat memantau KPI dan saran keuangan yang transparan.</div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
# ============================================================
# SISA FUNGSI RENDER UI & GRAFIK (TETAP SAMA SEPERTI ASLINYA)
# ============================================================
...
def render_tips(monthly, pred):
    ...
    tips.append({'badge':'N','w':CLR_GREEN,'judul':'Selalu Catat Setiap Transaksi',
        'isi':'Model Artificial Intelligence (LSTM) yang bekerja di latar belakang sangat bergantung pada data harian di Database. Catat semua transaksi Anda secara konsisten.'})
    cols = st.columns(2)
...
def main():
    ...
    df = fetch_data_from_supabase(supabase)
    
    # 2. Render Sidebar
    tahun_sel, bulan_sel = render_sidebar(df)
    
    # 3. Render Header Utama
    ...
    <div class="main-header">
        ...
        <div class="header-text">
            ...
            <p>Sistem Pemantauan Terintegrasi Database Real-time & Machine Learning &nbsp;·&nbsp; {tgl}</p>
        </div>
        <div class="header-badge">
            <span class="header-badge-dot"></span>SIKARTA AI v4.1
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if df.empty:
        ...
        <div class="footer-box">
            SIKARTA Dashboard &nbsp;·&nbsp; Sistem Terhubung ke Supabase
            <span>Menunggu data transaksi pertama Anda dari sistem...</span>
        </div>""", unsafe_allow_html=True)
        return
    ...
    st.markdown(f"""
    <div class="footer-box">
        SIKARTA Dashboard &nbsp;·&nbsp; Transparansi &amp; Kemajuan Bersama
        <span>Data Tahun {tahun_aktif} &nbsp;·&nbsp; Diperbarui {datetime.now().strftime('%d %B %Y pukul %H:%M WIB')} &nbsp;·&nbsp; v4.1 (Supabase ML Build)</span>
    </div>""", unsafe_allow_html=True)
